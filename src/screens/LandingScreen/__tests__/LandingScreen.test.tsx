...
describe('LandingScreen', () => {
  ...
  it('renders settings screen correctly', async () => {
    const {getByText, getByDisplayValue} = render(<LandingScreen />, {
      withSafeArea: true,
    });
  ...
  it('updates context size correctly', async () => {
    jest.useFakeTimers();
    const {getByDisplayValue} = render(<LandingScreen />, {
      withSafeArea: true,
    });
  ...
  it('displays error for invalid context size input', async () => {
    const {getByDisplayValue, getByText} = render(<LandingScreen />, {
      withSafeArea: true,
    });
    const contextSizeInput = getByDisplayValue('1024');
  ...
  it('toggles Auto Offload/Load switch', async () => {
    const {getByTestId} = render(<LandingScreen />, {withSafeArea: true});
